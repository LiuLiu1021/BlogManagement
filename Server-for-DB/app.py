...
def getNewBlog():
    ...
    records = []
    sql = "SELECT * FROM Blog WHERE state=0 ORDER BY publish_time DESC"
    cursor.execute(sql)
    row = cursor.fetchone()
...
def getBlogByUid():
    print(request.values.get("blog_id"))
    blog_id = int(request.values.get("blog_id"))
    sql = "SELECT * FROM Blog WHERE blog_id=" + str(blog_id)
    cursor.execute(sql)
    ...
    if len(row) == 0:
        ...
        if result['need_credit'] == 0 and user_info is not None:
            sql = 'INSERT INTO History VALUES("%s",%d,now())' % (user_info['user_id'], blog_id)
            cursor.execute(sql)
        ...
        sql = "UPDATE History SET time=now() WHERE user_id='%s' AND blog_id='%d'" % (user_info['user_id'], blog_id)
        print(sql)
        cursor.execute(sql)
        sql = 'UPDATE Blog SET browse_number=browse_number+1 WHERE blog_id=%d' % blog_id
        cursor.execute(sql)
...
def addComment():
    ...
    sql = 'INSERT INTO Comment(comment_id,user_id,blog_id,time,content) VALUES (%d,"%s",%d,now(),"%s")' % (
    comment_id, user_info['user_id'], blogUid, content)
    cursor.execute(sql)
...
def getCommentListByUser():
    ...
    sql = "SELECT Comment.comment_id,Comment.user_id,Comment.blog_id,Comment.time,Comment.content,Blog.user_id,Blog_user.name from Comment,Blog,Blog_user where Comment.blog_id=Blog.blog_id and Blog_user.user_id=Blog.user_id and Blog.user_id=" + user_id;
    print(sql)
    cursor.execute(sql)
    row = cursor.fetchone()
    print(row)
...
def getBlogSortList():
    data = {}
    records = []
    sql = "SELECT activity_id,name FROM Activity"
    cursor.execute(sql)
    row = cursor.fetchone()
    i = 0
    while row:
...
def addBlog():
    ...
    sql = "INSERT INTO Blog VALUES (" + str(blog_id) + ",\"" + user_info['user_id'] + \
          "\",now(),\"" + form.get("title") + "\",\"" + form.get("summary") + "\",\"" + \
...
def followByUid():
    ...
    row = cursor.fetchone()
    if row == None:
        sql = "INSERT INTO Follow VALUES ('" + user_info['user_id'] + "','" + uid + "',now())"
        print(sql)
        cursor.execute(sql)
    else:
...
def getArticleBySort():
    ...
    data = {}
    records = []
    sql = "SELECT Blog.user_id,publish_time,title,label,activity,name " \
          " FROM Blog,Activity " \
...
def addBlogSort():
    ...
    sql = "insert into Activity (activity_id,user_id,name,start_time,end_time,credit,description) values (" + (
            num + 1) + "," + user_info[
              'user_id'] + "," + activityName + "," + startTime + "," + endTime + "," + credit + "," + description + ")"
    cursor.execute(sql)
    row = cursor.fetchone()
    print(row)
    data['message'] = '添加活动成功'
...
def ReportBlog():
    ...
    print(id)
    print(content)
    sql = "INSERT INTO `blogs`.`Report` (`user_id`, `blog_id`, `time`, `reason`, `state`) VALUES ('%s', '%d', now(), '%s', 0);" % (
        user_info['user_id'], id, content)
    try:
        # 执行SQL语句
        cursor.execute(sql)
